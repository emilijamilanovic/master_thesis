...
def plot_menu_size(rows, prompt, out):
    ...
    fig.tight_layout()
    fig.savefig(out / f'{TAG}_menu_size.png', dpi=150)
    plt.close(fig)
...
def plot_dependence(rows, prompt, out):
    ...
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(out / f'{TAG}_dependence.png', dpi=150)
    plt.close(fig)
    return True
...
def main():
    ...
    for prompt in prompts:
        ...
        plot_dependence(group, prompt, out)
    ...
    print(f'\nWrote {TAG}_menu_size.png and {TAG}_dependence.png to {out}/')
